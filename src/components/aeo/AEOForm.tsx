...
export default function AEOForm() {
  ...
  const [declarations, setDeclarations] = useState([ { ...emptyDeclaration } ]);
  ...
  const [recordKeepings, setRecordKeepings] = useState([ { ...emptyRecord } ]);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 md:col-span-4">
          ...
          <input value={tin} onChange={(e) => setTin(e.target.value)} className="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800" placeholder="20202020" />
        </div>
      </div>
      ...
      <section className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => addItem(setDeclarations, emptyDeclaration)} className="text-sm text-brand-500">+ Add</button>
        </div>
        <div className="mt-3 space-y-3">
          {declarations.map((d, i) => (
            <div key={i} className="grid grid-cols-12 gap-3 items-end">
              <div className="col-span-12 md:col-span-3">
                ...
                <input value={d.declarantFullName} onChange={(e) => setDeclarations(s => { const c = [...s]; c[i].declarantFullName = e.target.value; return c; })} className="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800" />
              </div>
              <div className="col-span-12 md:col-span-3">
                <label className="text-sm">Capacity</label>
                <input value={d.declarantCapacity} onChange={(e) => setDeclarations(s => { const c = [...s]; c[i].declarantCapacity = e.target.value; return c; })} className="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800" />
              </div>
              <div className="col-span-12 md:col-span-3">
                <label className="text-sm">Declaration Date</label>
                <input type="datetime-local" value={new Date(d.declarationDate).toISOString().slice(0,16)} onChange={(e) => setDeclarations(s => { const c = [...s]; c[i].declarationDate = new Date(e.target.value).toISOString(); return c; })} className="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800" />
              </div>
              <div className="col-span-12 md:col-span-2 text-right">
                <button type="button" onClick={() => removeItem(setDeclarations, i)} className="text-red-500">Remove</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow grid grid-cols-1 gap-4">
        <div>
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => addItem(setOverseasPurchasers, { ...emptyOverseas[0] })} className="text-sm text-brand-500">+ Add</button>
          </div>
      ...
      {/* Record Keepings */}
      <section className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Record Keeping</h3>
